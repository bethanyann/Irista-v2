  ...
  const handleDrop = (acceptedFiles, rejectedFiles) => {
    ...
    if (rejectedFiles.length > 0) {
      ...
      if (rejectedFiles[0].errors[0].code === "too-many-files") {
        errorList.push(
          "Files could not be uploaded. File upload is limited to 15 photos at a time."
        );
        ...
        rejectedFiles.forEach((err) => {
          errorList.push([err.file.path, err.errors[0].message]);
        });
        setFileErrors(errorList);
      }
    }
    ...
    if (files.length === 0) {
      ...
      const uploadedData = files.map(async (file) => {
        try {
          ...
          const url = albumname
            ? `/api/uploadPhotos/${username}/${filename}/${albumname}`
            : `/api/uploadPhotos/${username}/${filename}`;

          return axios
            .post(url, { data: file.fileString })
            .then((response) => {
  ...
  return (
    <Wrapper>
          ...
          <section>
            ...
            <Content {...getRootProps()}>
              ...
              <h4 style={{ marginTop: 0 }}>
                *Individual file size limit of 10MB due to trial cloud storage
                account.*
              </h4>
                      ...
                      <div className="thumb" key={file.name}>
                        <div className="remove-checkbox">
                          <div className="remove-checkbox-span">
                            <div
                              className="checkbox"
                              onClick={() => handleDeletePhoto(file.name)}
                            >
                              X
                            </div>
                          </div>
                        ...
                        <div className="thumb-inner">
                          <img
                            ...
                            src={file.preview}
                            alt="thumbnail of the images to upload"
                            onLoad={() => {
      ...
      <div className="button-container">
        {error ? (
          <Alert style={{ marginRight: "20px" }} message={error} type="error" />
        ) : null}
        <Button
