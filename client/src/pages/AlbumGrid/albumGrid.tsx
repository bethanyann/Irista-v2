...
const AlbumGrid = () => {
    ...
    const handleOpenAlbum = (albumId: number) => {
        ...
        const albumIdEncoded = albumId.toString();
                ...
                <NewAlbumButton onClick={() => setAlbumModalIsOpen(true)}>New Album<span className='red-plus'>+</span></NewAlbumButton>
                {
                     !loading && data ?
                         data.getAlbums.map((album) => (
                            <AlbumThumbnail key={album.albumName} image={album.coverPhotoUrl ?? NoImage } onClick={() => handleOpenAlbum(album.albumId)}>{album.albumName}</AlbumThumbnail>
                         ))
                     : null
                }
            </Content>
