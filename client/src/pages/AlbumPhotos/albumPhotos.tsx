...
const AlbumPhotos = () => {
  ...
  const handleConfirmModal = () => {
    ...
    if (photos) {
      ...
      newArray.total_count = photos
        ? photos.resources.length + totalFiles.length
        : totalFiles.length;
  ...
  const handleDeletePhotos = async () => {
    ...
    if (selectedPhotos.size > 0) {
      ...
      await fetch(`/api/deletePhotos`, {
        ...
        .then((data) => {
          let newArray = {} as Photos;
          newArray.resources = photos!.resources.filter(
            (photo) => !selectedPhotoArr.find((em) => em === photo.public_id)
          );
  ...
  return (
    <>
      ...
      >
        <Result
          status="warning"
          title={`Delete ${selectedPhotos.size} photo${
      ...
      >
        <Result
          ...
          subTitle={
            totalFiles.length > 1
              ? totalFiles.length + " photos successfully uploaded."
              : totalFiles.length + " photo successfully uploaded."
          }
          extra={[
            <button
              key={1234}
